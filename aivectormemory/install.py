...
def _build_opencode_plugin_content() -> str:
    """动态生成 OpenCode 插件内容，DEV_WORKFLOW_PROMPT 为唯一数据源"""
    js_rules = DEV_WORKFLOW_PROMPT.replace('`', '\\`').replace('${', '\\${')
...
def _write_inject_workflow_script(target_dir: Path) -> tuple[Path, bool]:
    """生成 inject-workflow-rules.sh，内容从 DEV_WORKFLOW_PROMPT 动态生成"""
    target_dir.mkdir(parents=True, exist_ok=True)
    dst = target_dir / "inject-workflow-rules.sh"
    content = "#!/bin/bash\ncat <<'AIVECTORMEMORY_EOF'\n" + DEV_WORKFLOW_PROMPT + "\nAIVECTORMEMORY_EOF\n"
...
def _write_opencode_plugins(plugins_dir: Path) -> list[str]:
    ...
    # 写入插件文件
    plugin_content = _build_opencode_plugin_content()
...
def _write_steering(filepath: Path, mode: str, ide_name: str = "", include_workflow: bool = True) -> bool:
...
def run_install(project_dir: str | None = None):
    ...
    for idx in ide_indices:
        ...
        if steering_fn and steering_mode:
            ...
            s_changed = _write_steering(steering_path, steering_mode, ide_name, include_workflow)
