...
- `run doctor codex` 会验证 `codex mcp get`、推荐参数（`uvx -q --no-progress --from ... --project-dir .`）以及
  `initialize -> tools/list -> status -> auto_save` 探针链路。
- `run migrate-project` 会迁移所有含 `project_dir` 的表；遇到 `session_state` 冲突时自动合并。
